...
public class ArrayUtils {
    ...
    */
    ...
    public static double[][] createRandomDoubleMatrix(int rowCount, int colCount, double minValue){
        double[][] matrix = new double[rowCount][];
        for (int r = 0; r < rowCount; r++)
            matrix[r] = createRandomDoubleArray(colCount, minValue);
        return matrix;
    }
    public static double[] createRandomDoubleArray(int length, double minValue) {
        double[] arr = new double[length];
        for (int i = 0; i < length; i++)
            arr[i] = minValue + RND.nextDouble() +RND.nextInt(200) ;
        return arr;
    }
}
